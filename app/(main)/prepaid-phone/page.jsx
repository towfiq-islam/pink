...
const Page = () => {
  ...
  const handleToggleCompare = product => {
    setSelectedCompare(prev => {
      ...
      if (prev.length >= MAX_COMPARE) {
        toast.error(`You can only compare up to ${MAX_COMPARE} phones`);
        return prev;
      }
      return [...prev, product];
    });
  ...
  const handleCompare = () => {
    ...
    const ids = selectedCompare.map(p => p.id).join(",");
    // router.push(`/compare?ids=${ids}`);
  };

  return (
    <section
      className={`container mb-5 md:mb-8 lg:mb-12 xl:mb-20 transition-[padding] duration-300 ${
        selectedCompare.length > 0 ? "pb-24" : ""
      }`}
    >
            ...
            {productLoading ? (
              ...
              products.map(product => (
                <Product
                  ...
                  product={product}
                  isCompare={true}
                  isSelected={selectedCompare.some(p => p.id === product.id)}
